...
func updateAllDatabaseTablesStructure(c *core.CliContext) error {
	...
	err = datastore.Container.UserDataStore.SyncStructs(new(models.InvestmentPosition), new(models.InvestmentRevision), new(models.InvestmentPosting), new(models.InvestmentCommand), new(models.InvestmentDefinition), new(models.InvestmentPurchaseCategory), new(models.InvestmentDefinitionBinding), new(models.InvestmentAttachment), new(models.InvestmentValuationSetting))
	if err != nil {
		return err
	}

	err = datastore.Container.UserDataStore.SyncStructs(new(models.AccountGroup), new(models.AccountGroupMember), new(models.FundingPlan))
	if err != nil {
		return err
	}
