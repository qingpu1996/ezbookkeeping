<template>
 <section class="funding-panel" aria-label="资金规划">
  ...
  <template v-else>
   <p v-if="!plan.configured" class="notice">待设置：请明确选择可动用账户和保留金额，再计算投资可用资金。新账户默认不计入。</p>
   ...
   <div v-if="summary" class="figures">
    ...
    <div><small>保留资金</small><strong>{{ plan.configured ? money(summary.reserve) : '待设置' }}</strong></div>
   ...
   <small class="explanation">可调配资金 − 保留资金 = 投资可用资金（最低为零）。仅统计所设币种的资金余额；投资持仓、负债和其他币种不计入。请把近期生活费、还款等预留在保留资金中，未自动扣除全部负债。账户是否隐藏或从总额排除，不改变这里的明确选择。</small>
  </template>
  <dialog ref="dialog" @cancel="cancelEditor">
   <form @submit.prevent="save">
    ...
    <fieldset :disabled="busy">
     <label class="reserve-label">保留资金 · {{ currency }}<input v-model="reserveInput" inputmode="decimal" placeholder="请输入金额，允许明确填 0" required /></label>
     <small>在下方已勾选账户中再保留这笔钱。未勾选账户不会再次扣减。</small>
...
const dialog = ref<HTMLDialogElement>(); const selected = ref<string[]>([]); const reserveInput = ref('');
...
async function save() {
 ...
 editError.value = '';
 let reserve: string;
 try { reserve = investmentMinorUnits(reserveInput.value); } catch { editError.value = '请输入非负金额，最多两位小数。'; return; }
 const owner = user.currentUserBasicInfo;
 busy.value = true;
 try {
  const r = await services.saveFundingPlan({ currency: currency.value, reserve, accountIds: [...selected.value], version: plan.value.version });
