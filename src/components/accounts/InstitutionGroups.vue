        ...
        <template v-if="loaded">
            <article v-for="section in sections" :key="section.id" class="group-card">
                ...
                <div v-for="account in section.accounts" :key="account.id" class="account-row">
                    <div><h3>{{ account.name }}<small v-if="account.hidden"> · 已隐藏</small></h3><p>{{ tt(AccountCategory.valueOf(account.category)?.name || '') }}<span v-if="account.investmentPositionId"> · 黄金成本账户</span><span v-if="excluded[account.id]"> · 不计入汇总</span></p>
                        <strong v-if="account.type === 1">{{ accountMoney(account) }}</strong>
                        <ul v-else><li v-for="child in visibleChildren(account)" :key="child.id">{{ child.name }} · {{ accountMoney(child) }}<span v-if="child.hidden"> · 已隐藏</span><span v-if="excluded[child.id]"> · 不计入汇总</span></li></ul>
                    </div>
