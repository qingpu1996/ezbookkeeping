...
export function purchaseCategory(definitions:InvestmentDefinition[]):TransactionCategory {
 const group=TransactionCategory.createNewCategory(CategoryType.Expense);group.id=ASSET_PURCHASE_PREFIX;group.name='投资';group.icon='2101';group.visible=true;
 group.subCategories=definitions.map(d=>{const item=TransactionCategory.createNewCategory(CategoryType.Expense,group.id);item.id=ASSET_PURCHASE_PREFIX+d.id;item.name=d.name;item.icon='910';item.visible=true;return item});return group;
}
